...
get_private_addresses_for_linux_public_agents() {
    ...
    #
    VMSS_NAME=$(az vmss list -o table -g $AZURE_RESOURCE_GROUP | grep "dcos-$LINUX_AGENT_PUBLIC_POOL" | awk '{print $1}')
    az vmss nic list -g $AZURE_RESOURCE_GROUP --vmss-name $VMSS_NAME | python -c "import sys, json ; data = json.load(sys.stdin) ; v = [s['ipConfigurations'][0]['privateIpAddress'] for s in data] ; print('\n'.join(v))"
}
...
get_private_addresses_for_linux_private_agents() {
    ...
    #
    VMSS_NAME=$(az vmss list -o table -g $AZURE_RESOURCE_GROUP | grep "dcos-$LINUX_AGENT_PRIVATE_POOL" | awk '{print $1}')
    az vmss nic list -g $AZURE_RESOURCE_GROUP --vmss-name $VMSS_NAME | python -c "import sys, json ; data = json.load(sys.stdin) ; v = [s['ipConfigurations'][0]['privateIpAddress'] for s in data] ; print('\n'.join(v))"
}
